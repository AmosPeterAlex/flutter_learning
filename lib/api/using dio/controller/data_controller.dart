...
class DataController extends GetxController {
  RxList<DataModel> dataS = RxList(); // RxList for storing data, similar to observable list
  ...
  RxBool isNetConnected = true.obs; // RxBool for tracking internet connection status
  ...
  void isInternetConnected() async {
    isNetConnected.value = await InternetConnectionChecker().hasConnection; // Check internet connection status and update observable
  }
  ...
  fetchData() async {
    ...
    var response = await DioService().getData(url); // Make API call using DioService
    if (response.statusCode == 200) {
      response.data.forEach(
            (data) {
          dataS.add(
            DataModel.fromJson(data), // Parse API response and add data to RxList
          );
        },
      );
      isLoading.value = false; // Set loading state to false after data fetching is complete
    }
  }
